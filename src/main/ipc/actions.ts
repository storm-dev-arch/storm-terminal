...
export function registerActionHandlers(mainWindow: BrowserWindow): void {
  ...
  // Launch Windows Diagnostic Tool
  ...
  // Launch Admin Diagnostic (SFC, DISM, CHKDSK) in elevated prompt
  ...
  // Disk Benchmark (Non-destructive 64MB sequential & 4K random read/write in %TEMP%)
  ...
  // Set High Performance Plan (legacy shortcut)
...
====================================================`;
  ...
  ipcMain.handle('settings:save', (_event, patch: Partial<AppSettings>): AppSettings => {
    try {
      ...
      // Broadcast settings update to all active windows
