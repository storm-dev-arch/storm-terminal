...
// Cached static hardware info so we NEVER spawn heavy WMI/PowerShell processes in the fast tick!
let cachedCpuInfo: any = null;
let cachedGpuInfo: any = null;
let cachedPrimaryDisk: any = null;
let cachedPrimaryIface: any = null;

// Background low-frequency cache for GPU & Disk I/O & Temps
...
// Network bandwidth tracking via ultra-fast native Windows netstat counters
let lastRxBytes = 0;
let lastTxBytes = 0;
let lastNetRxSec = 0;
let lastNetTxSec = 0;
...
// Native CPU load sampling using os.cpus() deltas (takes ~0.15ms, ZERO subprocesses)
...
// Background network sampler: 0ms overhead on main loop
...
// Background slow polling for GPU & Disk & CPU dynamic freq
function startSlowSensorsWorker(isMoving: () => boolean) {
  // Network ticker every 1.5s
  setInterval(() => {
    sampleNetwork(isMoving);
  }, 1500);

  // Dynamic CPU Frequency ticker (every 2.5s) via WMI
  setInterval(() => {
    if (isMoving && isMoving()) return;
  ...
  // Fast NVIDIA-SMI query (every 2.5s) for real GPU temperature, usage & power
  setInterval(() => {
    if (isMoving && isMoving()) return;
  ...
  // Heavy sensors ticker every 4.5s
  setInterval(async () => {
    if (isMoving && isMoving()) return;
    ...
    try {
      ...
      if (cpuTemp.main && !isNaN(cpuTemp.main)) {
        // Discard bogus BIOS dummy ACPI temperatures <= 20°C (e.g. static 16.8°C / 17°C)
        if (cpuTemp.main > 20) {
...
export function registerSystemHandlers(isWindowMoving?: () => boolean): void {
  ...
  // Ultra-Fast Non-blocking Overview Telemetry (< 0.5ms round-trip)
  ipcMain.handle('system:getOverview', async (): Promise<SystemOverview> => {
    // If the window is actively being dragged/moved by the user, return last cached data INSTANTLY!
    // This guarantees 0ms delay during window drag.
    if (isMoving() && lastOverview) {
      return lastOverview;
    }

    try {
      // Instant native C++ OS memory queries (0.01 ms execution time, zero subprocesses!)
      const totalMem = os.totalmem();
      const freeMem = os.freemem();
      const usedMem = totalMem - freeMem;
      const memPct = Math.min(100, Math.max(0, Math.round((usedMem / totalMem) * 1000) / 10));

      // Instant native CPU snapshot delta (0.15 ms execution time)
      sampleCpu();
  ...
  // Detailed Hardware Specs (only invoked when System page is viewed)
  ...
  // Detailed Storage Partitions Handler
